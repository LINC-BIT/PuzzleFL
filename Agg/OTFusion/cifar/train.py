...
import Agg.OTFusion.cifar.models as models
import Agg.OTFusion.cifar.cifar_utils.accumulators as accumulators
...
def main(config, output_dir, gpu_id, pretrained_model=None, pretrained_dataset=None, tensorboard_obj=None, return_model=False):
    ...
    best_accuracy_so_far = accumulators.Max()
    ...
    for epoch in range(config['num_epochs']):
        ...
        mean_train_accuracy = accumulators.Mean()
        ...
        # Evaluation
        model.eval()
        mean_test_accuracy = accumulators.Mean()
        mean_test_loss = accumulators.Mean()
...
def get_model(config, device=-1, relu_inplace=True,output=100):
    """
    ...
    model = {
        ...
        'sixcnn':lambda :models.SixCNN([3,32,32],outputsize=output),
        'textcnn':lambda :models.Noemb_TextCNN(),
        'GCN':lambda :models.GCN()
    }[config['model']]()
