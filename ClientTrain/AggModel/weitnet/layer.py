...
class DecomposedConv(nn.Module):
    ...
    def get_weight(self):
        ...
        weight = (sw * m(self.mask)).transpose(0, -1) + self.aw + torch.sum(self.atten * self.from_kb.to(CONST.DEVICE), dim=-1)
        weight = weight.type(torch.cuda.FloatTensor)
        return weight
...
class DecomposedLinear(nn.Module):
    ...
    def get_weight(self):
        ...
        # newmask = m(self.mask)
        # print(sw*newmask)
        # print('atten shape: ', self.atten.shape)
        # print('from_kb shape: ', self.from_kb.shape)
        # print('aw shape: ', self.aw.shape)
