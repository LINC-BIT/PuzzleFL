...
The pa
per mention the value of 100 for SimpleCNN. It should be set to 512 in case of resnet-18"""
if ARCH == 'resnet':
    assert CONV_CH == 512

EPOCHS_PER_TASK = 21
SPARSITY_PATIENCE_EPOCHS = 5
LAMBDA_SPARSE = 0.1
...
FREEZE_TOP_PROC = 0.1  # used with "in top k%" strategy
FREEZE_PROB_THR = 0.0005  # used with "was chosen with probability > thr" strategy
...
# Experiment metadata
EXPERIMENT_TAG = 'sigmoid_lambda1e-1_100ep'
...
ROOT_DIR = Path('D:\\LPYX\\Code\\Federated Continual Learning\\MyCode\\FedAgg')
