...
def noniid(dataset, num_users, shard_per_user, num_classes, rand_set_all=[],dataname='cifar100'):
    ...
    if len(rand_set_all) == 0:
        ...
        for i in range(num_users):
            cur_all_label = [i for i in range(10)]
            for label in range(num_classes):
                if label_num[label] >= shard_per_class:
                    cur_all_label.remove(label)
            
            select_label = list(np.random.choice(cur_all_label, shard_per_user, replace=False))
            rand_set_all.append(select_label)
            for sl in select_label:
                label_num[sl] += 1
