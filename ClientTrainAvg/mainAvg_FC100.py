...
from ClientTrainAvg.LongLifeMethod.Avg_EWC import Appr as EWC_Appr
from ClientTrainAvg.LongLifeMethod.Avg_EWC import LongLifeTrain as EWC_LongLifeTrain
from ClientTrainAvg.LongLifeMethod.Avg_GEM import Appr as GEM_Appr
from ClientTrainAvg.LongLifeMethod.Avg_GEM import LongLifeTrain as GEM_LongLifeTrain
from ClientTrainAvg.LongLifeMethod.Avg_FedKNOW import Appr as FedKNOW_Appr
from ClientTrainAvg.LongLifeMethod.Avg_FedKNOW import LongLifeTrain as FedKNOW_LongLifeTrain
from ClientTrainAvg.LongLifeMethod.Avg_MAS import Appr as MAS_Appr
from ClientTrainAvg.LongLifeMethod.Avg_MAS import LongLifeTrain as MAS_LongLifeTrain
from ClientTrainAvg.LongLifeMethod.Avg_Packnet import Appr as Packnet_Appr
from ClientTrainAvg.LongLifeMethod.Avg_Packnet import LongLifeTrain as Packnet_LongLifeTrain
from ClientTrainAvg.LongLifeMethod.Avg_ChannelGate import Appr as ChannelGate_Appr
from ClientTrainAvg.LongLifeMethod.Avg_ChannelGate import LongLifeTrain as ChannelGate_LongLifeTrain
...
if __name__ == '__main__':
    ...
    print(client_task)
    print(args.alg)
    write = SummaryWriter('/data/lpyx/FedAgg/ClientTrainAvg/log/FC100/graph'+args.clmethod+'/server_epoch10_high20_dag_' + args.dataset+'_'+'round' + str(args.round) + '_frac' + str(args.frac))
    # build model
    ...
    if args.clmethod == 'EWC':
        apprs = [EWC_Appr(copy.deepcopy(net_glob).to(args.device), None,lr=args.lr, nepochs=args.local_ep, args=args) for i in range(args.num_users)]
    elif args.clmethod == 'MAS':
        apprs = [MAS_Appr(copy.deepcopy(net_glob).to(args.device), None, lr=args.lr, nepochs=args.local_ep, args=args) for i in range(args.num_users)]
    elif args.clmethod == 'GEM':
        apprs = [GEM_Appr(net_glob.to(args.device), 3 * 32 * 32, 100, 10, args) for i in range(args.num_users)]
    elif args.clmethod == 'FedKNOW':
        apprs = [FedKNOW_Appr(copy.deepcopy(net_glob),PackNet(args.task,device=args.device),copy.deepcopy(net_glob), None,lr=args.lr, nepochs=args.local_ep, args=args) for i in range(args.num_users)]
    elif args.clmethod == 'Packnet':
        apprs = [Packnet_Appr(copy.deepcopy(net_glob).to(args.device), None, lr=args.lr, nepochs=args.local_ep, args=args) for i in range(args.num_users)]
    elif args.clmethod == 'ChannelGate':
        apprs = [ChannelGate_Appr(copy.deepcopy(net_glob_cl).to(args.device), None,lr=args.lr, nepochs=args.local_ep, args=args) for i in range(args.num_users)]
    print(args.round)
    ...
    for iter in range(args.epochs):
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            te_dataloaders = DataLoader(DatasetSplit(dataset_test[client_task[idx][task]],dict_users_test[idx],tran_task=[task,client_task[idx][task]]),batch_size=args.local_bs, shuffle=True)
                # if args.epochs == iter:
            ...
            appr.set_trData(tr_dataloaders,te_dataloaders)
        ...
        if args.clmethod == 'ChannelGate':
            ...
            acc_test, loss_test = test_img_local_all(None, args, dataset_test, dict_users_test, task, apprs=apprs,
                                                     ...
                                                     client_task=client_task,spe=True)
