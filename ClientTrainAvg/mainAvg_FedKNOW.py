...
from ClientTrainAvg.LongLifeMethod.Avg_FedKNOW import Appr,LongLifeTest,LongLifeTrain
...
if __name__ == '__main__':
    ...
    print(args.alg)
    write = SummaryWriter('/data/lpyx/FedAgg/ClientTrainAvg/log/FedKNOW/FedKNOW_server_avg_' + args.dataset+'_'+'round' + str(args.round) + '_frac' + str(args.frac))
    # build model
        ...
        if iter % args.round == args.round - 1:
            for i, appr in enumerate(apprs):
                if len(appr.pack.masks) <= task:
                    ...
                    tr_dataloaders = DataLoader(DatasetSplit(dataset_train[client_task[i][task]],dict_users_train[i][:args.m_ft],tran_task=[task,client_task[i][task]]),batch_size=args.local_bs, shuffle=True)
