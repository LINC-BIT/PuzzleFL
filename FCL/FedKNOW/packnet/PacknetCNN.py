...
class PackNetCNN():
    ...
    def __init__(self, n_tasks, local_ep,local_rep_ep,prune_instructions=.9, prunable_types=(nn.Conv2d, nn.Linear), device =None, net=None):
        """
    ...
    def prune(self, t, model, prune_quantile):
        ...
        # Calculate Quantil based on current prunable params
        ...
        for name, param_layer in model.named_parameters():
            if 'bias' not in name:
                ...
                if p is not None:
                    all_prunable = torch.cat((all_prunable.view(-1), p), -1)    # 1-D vector
        B = torch.abs(all_prunable.cpu()).detach().numpy()
        cutoff = np.quantile(B, q=prune_quantile)

        # build mask for this task
        mask = {}
        with torch.no_grad():
            for name, param_layer in model.named_parameters():
                if 'bias' not in name:
    ...
    def training_mask(self, model):
        ...
        Apply this mask after .backward() and before optimizer.step() at every batch of training a new task
        """
        if len(self.masks) == 0:
            return
        for name, param_layer in model.named_parameters():
    ...
    def apply_eval_mask(self, model, task_idx):
        ...
        """
        # print(f'Applying eval mask for task {task_idx}, mask len {len(self.masks)}')
