...
class Appr():
    ...
    def _train_epoch(self, t, replay=False):
        ...
        for images, targets in self.tr_dataloader:
            ...
            if len(old_tids) > 0 and replay:
                grads = torch.Tensor(sum(self.grad_dims), len(old_tids)+1).to(self.args.device)     # grads 包含了旧任务以及新任务
            ...
            if len(old_tids) > 0 and replay:
                ...
                for idx, tt in enumerate(old_tids):
                    ...
                    ptloss.backward()
                    store_grad(self.model.parameters, grads, self.grad_dims, idx)
            ...
            if len(old_tids) > 0 and replay:
                cur_t_idx = len(old_tids)
                store_grad(self.model.parameters, grads, self.grad_dims, cur_t_idx)
                
                indx = torch.LongTensor(range(len(old_tids)))
                indx = indx.to(self.args.device)
                a = grads[:, cur_t_idx].unsqueeze(0)
                b = grads.index_select(1, indx)
                dotp = torch.mm(a, b)
                if (dotp < 0).sum() != 0:
                    # project2cone2(grads[:, t].unsqueeze(1), grads.index_select(1, indx), self.margin)
                    project2cone2(grads[:, cur_t_idx].unsqueeze(1), b, self.margin)
                    overwrite_grad(self.model.parameters, grads[:, cur_t_idx], self.grad_dims)
