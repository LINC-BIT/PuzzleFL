...
class Appr():
    ...
    def observe(self, x, t, y):
        # self.opt = optim.Adam(self.model.parameters(), self.lr)   # ZUO: optimizer 位置放错了
        # update memory
        if t != self.old_task:
    ...
    def set_cell_neibors(self,cell_neighbors):
            self.cell_neighbors = cell_neighbors
    
    def get_cell_neibors(self):
...
def life_experience(task,appr,tr_dataloader,epochs,sbatch=10,args=None):
    ...
    appr.opt = optim.Adam(appr.model.parameters(), appr.lr) # ZUO: optimizer 位置放错了
