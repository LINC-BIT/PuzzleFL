...
class Appr(object):
    ...
    def train(self, t):
        ...
        for e in range(self.nepochs):
            # Train
            if 'cross' in self.args.alg:    # Only for test ewc
                self.train_epoch_rep_local(t, e)
            else:
                self.train_epoch_rep(t, e)
            # ts_time_state = time.time()
            # train_loss, train_acc = self.eval(t)
            # ts_time_end = time.time()
            # print(f'Eval time: {ts_time_end - ts_time_state}')
        ...
        # return train_loss, train_acc  # ZUO: 降低训练开销
        return 0, 0

    def train_epoch_rep_local(self, t, epoch, kd_lambda = 0.0):
        self.model.train()
        e_start = time.time()
        num_b = 0
        # Loop batches
        for images,targets in self.tr_dataloader:
            ...
            print(f'batch {num_b} time: {b_time_end - b_time_start}')
            num_b += 1
        e_time_end = time.time()
        
        print('epoch time: ', e_time_end - e_start)
